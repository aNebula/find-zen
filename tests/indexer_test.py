...
def test_index_builder(sample_users: list) -> None:
    expected_user_index ={
        ...
        'user_index_by_organization_id': {'1000': [1], 'None': [2]},
...
def test_populated_org_index_with_users_tickets(sample_users: list, sample_orgs: list, sample_tickets: list) -> None:
    users = Users.parse_obj(sample_users)
    ...
    org_index_populated = populated_org_index_with_users_tickets(
        user_index_pre_population,
        orgs_index_pre_population,
    ...
    assert org_index_populated['organization_index_by_id']['1000']['users'] == org1000_expected_user
    assert org_index_populated['organization_index_by_id']['2000']['users'] == []
    assert org_index_populated['organization_index_by_id']['1000']['tickets'].sort(key=lambda x: x['id']) == org1000_expected_tickets.sort(key=lambda x: x.id)
    assert org_index_populated['organization_index_by_id']['2000']['tickets'] == []


def test_populate_user_index_with_org_tickets(sample_users: list, sample_orgs: list, sample_tickets: list) -> None:
    users = Users.parse_obj(sample_users)
    user_index_pre_population = index_builder(users, User)
    ...
    assert user_index_populated['user_index_by_id']['1']['organization'] == user1_expected_org
    assert user_index_populated['user_index_by_id']['2']['organization'] == user2_expected_org
    assert user_index_populated['user_index_by_id']['1']['tickets'].sort(key=lambda x: x['id']) == user1_expected_tickets.sort(key=lambda x: x.id)
    assert user_index_populated['user_index_by_id']['2']['tickets'] == user2_expected_tickets


def test_populate_ticket_index_with_user_org(sample_users: list, sample_orgs: list, sample_tickets: list)-> None:
    users = Users.parse_obj(sample_users)
    user_index_pre_population = index_builder(users, User)
    ...
    assert ticket_index_populated['ticket_index_by_id']['Tix1']['user'] == ticket1_expected_user
    assert ticket_index_populated['ticket_index_by_id']['Tix2']['user'] == ticket2_expected_user
    assert ticket_index_populated['ticket_index_by_id']['Tix1']['organization'] == ticket1_expected_org
    assert ticket_index_populated['ticket_index_by_id']['Tix2']['organization'] == ticket2_expected_org