...
def test_search_cache() -> None:
    ...
    with mock.patch('findzen.file_handler.CacheHandler.read_cache', return_value = one_item):
        item = search_cache('user', 'id', ['1'])
        assert item == one_item['1']

    # happy path - multiple items found from 1 query
    with mock.patch('findzen.file_handler.CacheHandler.read_cache', return_value = two_items):
        item = search_cache('user', 'id', ['1'])
    ...
    with mock.patch('findzen.file_handler.CacheHandler.read_cache', return_value = two_items) as cache_miss:
        cache_miss.side_effect = FileNotFoundError('This particular data and/or field has not been loaded. Load data first using `load` command')
