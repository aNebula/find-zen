"""Pytest fixtures for commonly used routines in testing."""
import pytest
import json
import pathlib
import os
...
def sample_users():
    ...
    user1 = User(
        ...
        organization_id='1000', tags=['tag1', 'tag2'], suspended=False, role='admin'
        )
    user2 = User(
...
def sample_data_dir(tmp_path, sample_users, sample_orgs, sample_tickets):
    """Set up a temporary working directory with data directory."""
...
def tmp_cwd(tmp_path):
    """Set up a temporary working directory to execute tests."""
